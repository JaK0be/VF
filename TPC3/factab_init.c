...
/*@ requires \valid(factable+(0..size-1)) && size>0;
  ...
  @ ensures \forall integer a ; 0 <= a < size ==>  factable[a] == fact (a);
  @*/
void factab (int factable[], int size) {
  int k = 0 ;
  /*@ loop invariant
    @ 0 <= k <= size && (\forall integer a ; 0 <= a < k ==> factable[a] == fact (a)));
    @ loop assigns k, factable[0..size-1];
    @ loop variant size-k;
    @*/
  while (k < size) {
    factable[k] = factf(k); k++;
  } 
}
...
void factab_efficient (int factable[], int size) {
  ...
  factable[0] = 1;
  /*@ loop invariant
    @ 1 <= k <= size && (\forall integer a ; 0 <= a < k ==> factable[a] == fact (a));
